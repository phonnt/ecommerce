...
| `apps/app/vercel.json` | Build admin trong Vercel Root Directory `apps/app` và rewrite SPA |
| `apps/storefront/vercel.json` | Build storefront static export trong Vercel Root Directory `apps/storefront` |
...
CI không tự deploy Railway. Hai Vercel projects được Git-connect với repo và production branch
`main`; push lên `main` có thể tạo production deployment qua Vercel Git integration.
...
Cần tạo hai Vercel projects riêng cùng trỏ tới repo:

1. `ecommerce-app` với Root Directory `apps/app`
2. `ecommerce-storefront` với Root Directory `apps/storefront`

Git-connect cả hai project với repo GitHub và đặt production branch là `main`. Với cấu hình đó,
push lên `main` tạo production deployment qua Vercel Git integration; push branch khác tạo preview
deployment nếu project không tắt preview deployments.
...
`apps/app/vercel.json` output ra `dist` trong Root Directory `apps/app` và có rewrite SPA về
`index.html`, nên route như
...
`apps/storefront/vercel.json` output ra `out` trong Root Directory `apps/storefront`. Khi build
thành công, log Next.js nên
